...
def fitness(im1, im2):
    ...
    # 1 : dif = numpy.sum(numpy.abs(i1 - i2)**2)
    # 2 : dif = numpy.sum(numpy.abs(i1 - i2))**2
    # 3 : dif = numpy.sum(numpy.sqrt(numpy.abs(i1**2 - i2**2)))
    # 4 : dif = numpy.sqrt(numpy.sum(numpy.abs(i1 - i2)**2))
