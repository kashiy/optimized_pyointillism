...
def fitness(im1, im2):
    ...
    i1 = numpy.array(im1, numpy.int64)
    i2 = numpy.array(im2, numpy.int64)
    dif = numpy.sum(numpy.abs(i1 - i2) ** 2)
    return (dif / 255.0 * 100) / i2.size
...
def run(cores, so=None):
    ...
    while generation <= 40000:
        ...
        try:
            ...
            results_mutate = select_and_mutate(parents)
        ...
        if len(results_mutate) > 0:
            mutate_scores, mutate_parents = zip(*results_mutate)
            # update parents and scores with all new operations results
            parents += mutate_parents
            scores += mutate_scores

        if len(results_crossover) > 0:
            crossover_scores, crossover_parents = zip(*results_crossover)
            # update parents and scores with all new operations results
            parents += crossover_parents
            scores += crossover_scores
