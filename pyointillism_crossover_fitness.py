...
def run(cores, so=None):
    ...
    while True:
        ...
        parents_select, scores_select = zip(*winners)

        # crossover from 50 mutations of 1 individual
        parent, score = make_crossover(parents_select, scores_select, p)[0]
...
def make_crossover(parents, scores, p):
    children = []
    num = 0
    while num < 50:
        a = parents[num].crossover(parents[num + 1])
        if a is not None:
            c1, c2 = a[0], a[1]
            children.append(c1)
            children.append(c2)
    ...
    parents += children
    results = p.map(test_fitness, parents)
    newScores, newChildren = zip(*results)
    winners = sorted(zip(newChildren, newScores), key=lambda x: x[1])
    return winners
